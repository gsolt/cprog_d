...
* V4.0	2008.08.01	Zsolt Gergely	Idõbélyeg fogadás MOSCAD állomás esetén 	*
* V4.1	2008.10.12	Zsolt Gergely	Gyõr, Árkád beillesztése 	*
...
short          *p_col_EVT;
short          *p_col_SC;
unsigned short short          *p_col_RxBuf;
   ...
   if(MOSCAD_RcvFrm(&site_inx, rx_buffer, &buff_len, &type) == 0)
   {
      ...
      switch (type)
      {
        ...
        case CB_MSG_TYPE:
			...
			if (site_inx==121) /*Gyor ETO*/
			{
			fnWriteSPData(sCP.sCPR[229].nSPOffsetCS, 0,  0,0,0,0);   			
			fnWriteSPStatus(sCP.sCPR[229].nSPOffsetCS, TOPICAL);
			...
			if (site_inx==121) /*Gyor ETO*/
			{
				fnWriteSPData(sCP.sCPR[229].nSPOffsetLek, 0,   0,0,0,0);
				fnWriteSPStatus(sCP.sCPR[229].nSPOffsetLek, TOPICAL);
				...
				if (  (site_inx == 121) || (site_inx == 229) )
 				{
 					if (nRxBuf[0] == 2048)
 					{
 		 				MOSCAD_sprintf(message,"ETO event 1.");
		   			 	MOSCAD_error(message ); 				

	 					TALUS_EVENT(&sTAL[121],rx_buffer);
 					}
 					if (nRxBuf[0] == 2049)
 					{
 		 				MOSCAD_sprintf(message,"ETO event 2.");
		   			 	MOSCAD_error(message ); 	
		   			 				
 						TALUS_EVENT(&sTAL[229],rx_buffer);
 					}
 				}		
				...
				if (  (site_inx == 121) || (site_inx == 229) )
 				{
					...
					if (nRxBuf[30] == 1)
 					{
		 				MOSCAD_sprintf(message,"ETO data 1.");
		   			 	MOSCAD_error(message ); 	
		   			 	
	 					TALUS_DAT(&sTAL[121],rx_buffer);
 					}
 					if (nRxBuf[30] == 2 )
 					{
 						TALUS_DAT(&sTAL[229],rx_buffer);
 						
		 				MOSCAD_sprintf(message,"ETO data 2.");
		   			 	MOSCAD_error(message ); 	
   ...
   for (nI=0;nI<sCP.nRtuNum && nI<MAX_RTU;nI++)
   {
   	for (nJ=sCP.sCPR[nI].nDCStart;nJ<sCP.sCPR[nI].nDCStart+sCP.sCPR[nI].nDCNum;nJ++)
   	{
   		...
   		if (p_col_DCAct[nJ-nOffset]>0)
   		{
			...
			if ( (nI!=121) && (nI!=229) &&  sTAL[nI].nLeagNum<9 )
			{
    		...
    		else if (nI==121)
   			{
   				...
   				{
					MOSCAD_sprintf(message,"ETO1. parancs");
   				 	MOSCAD_error(message ); 				
   			...
   			else if (nI==229)
   			{
   				...
   				{
					MOSCAD_sprintf(message,"ETO2. parancs");
   				 	MOSCAD_error(message ); 				
...
if (nI == 117) /*Gyõr, Árkád*/
	{
			/* Egy bites jelzesek ------------------------------------*/
			for (nJ=2369;nJ<2496;nJ++)
			{
				fnWriteSPStatus( nJ, nValue);
			}
			/* Ket bites jelzesek --------------------------------*/
			for (nJ=367;nJ<373;nJ++)
			{
				fnWriteDPStatus( nJ, nValue);
			}
			/* Meresek  --------------------------------*/
			for (nJ=372;nJ<378;nJ++)
			{
...
/****************************************************************************/
/* Gyõr, Árkád állomás felõl érkezõ távirat feldolgozása						*/
/****************************************************************************/
void ARKAD_DAT( unsigned char *rx_buf)
{
int		nI,nJ;				
...
int		nOffs[10] = {0,8,16,24,40,48,56,64,80,96};
int		nBitNum[10] = {8,8,8,16,8,8,8,16,16,16};
...
/* Mérések feldolgozása ----------------------------------------------------------------------------------------*/
	...
	for (nI=0; nI<6 ; nI++)
	{
		fnWriteNM( nNMStart+nI,nRxBuf[20+nI]);
	} /*end for*/


/* Egybites jelzések feldolgozása ----------------------------------------------------------------------------------------*/
memcpy(&nDIBuf[0],&nRxBuf[3],6);
memcpy(&nDIBuf[3],&nRxBuf[7],2);

memcpy(&nDIBuf[4],&nRxBuf[9],6);
memcpy(&nDIBuf[7],&nRxBuf[13],2);


memcpy(&nDIBuf[8],&nRxBuf[14],2);
memcpy(&nDIBuf[9],&nRxBuf[19],2);


for (nJ=0; nJ<10; nJ++)
	{	
	for (nI=0; nI<nBitNum[nJ]; nI++)
	{
		
		nIEC_Offset = sSep[0].nIEC_SP+nOffs[nJ]+nI;
...
/*Mixed I/O DI-k ******************************************************************************************************/
	for (nI=0; nI<8; nI++)
	{
		...
		nIEC_Offset = sSep[0].nIEC_SP + 117 + nI;
		nData = nRxBuf[34];
		...
		if (nVal > 0)
		{
        MOSCAD_sprintf(message,"Arkad, DI, nIEC_Offset: %d",nIEC_Offset);
        MOSCAD_error(message );
...
/* Ketbites jelzesek**********************************************************/
	...
	/*Földelõ szakaszolók állásjelzései, 1. leágazás ********************/
	nDPStart = 	sSep[0].nIEC_DP_FSZ1;
	nData = nRxBuf[3];	
	...
	{ 
			...
			if (    ( (nData >> 0) & 1) == 1      )
			{
 				p_col_DPH[nDPStart-nMOSCAD_OffsetDP] = 1;
	 			p_col_DPL[nDPStart-nMOSCAD_OffsetDP] = 0;
			}
			else
			{
 				p_col_DPH[nDPStart-nMOSCAD_OffsetDP] = 0;
	 			p_col_DPL[nDPStart-nMOSCAD_OffsetDP] = 1;
			}
			
			p_col_DP_CT[nDPStart-nMOSCAD_OffsetDP]= 0;	
	...
	/*Földelõ szakaszolók állásjelzései, 2. leágazás ********************/
	nDPStart = 	sSep[1].nIEC_DP_FSZ1;
	nData = nRxBuf[9];	
	...
	/*Földelõ szakaszolók állásjelzései, 3. leágazás ********************/
	nDPStart = 	sSep[2].nIEC_DP_FSZ1;
	nData = nRxBuf[19];	
...
nRxBuf = (unsigned short *)rx_buf;
	...
	MOSCAD_sprintf(message,"Gyõr, Árkád event: nSepamNum: %d,  nEventAddr: %dn nValue: %d",  nSepamNum, strTE[0].nTalusAddr, strTE[0].nValue);
    MOSCAD_error(message );
...
for (nI=0;nI<nEventNum && nI<4; nI++)
	...
	if (	sSep[nSepamNum].nIEC_SP > 0)
		{
		if (nSepamNum<2) /*SEPAM 2000*/
			{
			/* Ha egybites jelzés jött -------------------------------------------------------------------------------*/
			
			if (strTE[nI].nTalusAddr >= (49408+16) && strTE[nI].nTalusAddr < 49408 + 24 )
				{
					
					
				nData = strTE[nI].nValue;		
				nIEC_Offset = strTE[nI].nTalusAddr - (49408 + 16 ) + sSep[nSepamNum].nIEC_SP;
				
				fnWriteSPData(nIEC_Offset,nData ,strTE[nI].nMs1,strTE[nI].nMs2,strTE[nI].nMin,1);				
				
        MOSCAD_sprintf(message,"nSepamNum: %d, nData: %d, nIEC_Offset: %d", nSepamNum, nData, nIEC_Offset);
        MOSCAD_error(message );

							
				}/*end if egybites */		
			if (strTE[nI].nTalusAddr >= (49408 + 32) && strTE[nI].nTalusAddr < 49408 + 40 )
				{
				nData = strTE[nI].nValue;		
				nIEC_Offset = strTE[nI].nTalusAddr - (49408 + 24 ) + sSep[nSepamNum].nIEC_SP;			
				fnWriteSPData(nIEC_Offset,nData ,strTE[nI].nMs1,strTE[nI].nMs2,strTE[nI].nMin,1);
        ...
        MOSCAD_error(message );
			...
			if (strTE[nI].nTalusAddr >= (49408 + 48) && strTE[nI].nTalusAddr < 49408 + 56 )
				{
				nData = strTE[nI].nValue;		
				nIEC_Offset = strTE[nI].nTalusAddr - (49408+32) + sSep[nSepamNum].nIEC_SP;	
        ...
        MOSCAD_error(message );
			...
			if (strTE[nI].nTalusAddr >= 51456 && strTE[nI].nTalusAddr < 51456 + 16 )
				{
				nData = strTE[nI].nValue;		
				nIEC_Offset = strTE[nI].nTalusAddr - (51456-24 ) + sSep[nSepamNum].nIEC_SP;	
		...
		if (nSepamNum==2) /*SEPAM S20*/
			{
			/* Ha egybites jelzés jött -------------------------------------------------------------------------------*/
			
			if (strTE[nI].nTalusAddr >= (4096) && strTE[nI].nTalusAddr < 4096 + 16 )
				{
				nData = strTE[nI].nValue;		
        ...
        MOSCAD_error(message );
			...
			if (strTE[nI].nTalusAddr >= (4176) && strTE[nI].nTalusAddr < 4176 + 16 )
				{
				nData = strTE[nI].nValue;		
				nIEC_Offset = strTE[nI].nTalusAddr - (4176-16) + sSep[nSepamNum].nIEC_SP;
		...
		/* Ha valodi kétbites jelzés jött -----------------------------------------------------------------------------------*/
      if (nSepamNum<2) /*SEPAM 2000*/
      {
		if (strTE[nI].nTalusAddr >= 49408 && strTE[nI].nTalusAddr < 49408 + 2 )
		{
			
			/* Double point  */

			nIEC_Offset = (strTE[nI].nTalusAddr - 49408) / 2 + sSep[nSepamNum].nIEC_DP;						
        ...
        MOSCAD_error(message );*/
			...
			/* Érték beírása */
			/* Ha páros */
			if ( ((strTE[nI].nTalusAddr - 49408) & 1) == 0 )
			{
				p_col_DPH[nIEC_Offset-nMOSCAD_OffsetDP] = strTE[nI].nValue;	/* DPH -> CLOSE */			
			}
			if ( ((strTE[nI].nTalusAddr - 49408) & 1) == 1 )  /* DPL -> OPEN */
			{
				p_col_DPL[nIEC_Offset-nMOSCAD_OffsetDP] = strTE[nI].nValue;	
			}
			

        MOSCAD_sprintf(message,"nSepamNum: %d, strTE[nI].nValue: %d, nIEC_Offset: %d", nSepamNum, strTE[nI].nValue, nIEC_Offset);
        MOSCAD_error(message );
      ...
      if (nSepamNum==2) /*SEPAM S20*/
      {
		if (strTE[nI].nTalusAddr >= 4176 && strTE[nI].nTalusAddr < 4176 + 2 )
		{
			
			/* Double point  */

			nIEC_Offset = (strTE[nI].nTalusAddr - 4176) / 2 + sSep[nSepamNum].nIEC_DP;						
        ...
        MOSCAD_error(message );*/
			...
			/* Érték beírása */
			/* Ha páros */
			if ( ((strTE[nI].nTalusAddr - 4176) & 1) == 0 )
			{
				p_col_DPL[nIEC_Offset-nMOSCAD_OffsetDP] = strTE[nI].nValue;	/* DPL -> OPEN */			
			}
			if ( ((strTE[nI].nTalusAddr - 4176) & 1) == 1 )  /* DPH -> CLOSE */
			{
				p_col_DPH[nIEC_Offset-nMOSCAD_OffsetDP] = strTE[nI].nValue;	
			}
...
/* Ha 1 bitbõl képzett kétbites jelzés jött, foldelo szakaszolo ----------------------------------------------------------------------------*/
      ...
      if (nSepamNum<2) /*SEPAM 2000 -------------------------------------------------------*/
      {
		...
		if (           (strTE[nI].nTalusAddr == (49408+16)) && (sSep[nSepamNum].nIEC_DP_FSZ1!=0)              )
		{
			/* Double point  */
			nIEC_Offset = strTE[nI].nTalusAddr - (49408+16)  + sSep[nSepamNum].nIEC_DP_FSZ1;	
     ...
     if (nSepamNum==2) /*SEPAM S20 --------------------------------------------------------*/ 
      {
		if (   (strTE[nI].nTalusAddr == 4182) && (sSep[nSepamNum].nIEC_DP_FSZ1!=0)         )
		{
			/* Double point  */
			nIEC_Offset = strTE[nI].nTalusAddr - (4182)  + sSep[nSepamNum].nIEC_DP_FSZ1;			
...
TMOKAllasjelzesOffsetek[0] = 317; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[0][0] =317;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
...
nReteszOffset[0] = 0;             /* DC parancs és reteszfunkció DP állapot offsete, ha tartozik hozzá DC parancs*/
																													...
																													/**/
/**********************************************************************************************************************/																													/**/
/* 1. TMOK: 60-84 RTU: Front end H -> Szil, erõmû			 -----------------------*/								/**/
TMOKAllasjelzesOffsetek[1] = 281; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[1][0] =1250;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[1][0] = 242;			/* Front end H */															/**/
...
nReteszOffset[1] = 1;             /* DC parancs és reteszfunkció DP állapot offsete, ha tartozik hozzá DC parancs*/
																													/**/
/**********************************************************************************************************************/																													/**/
/* 2. TMOK: 10-46 RTU: Front end F -> Nagyszentjános, Ács			 -----------------------*/								/**/
TMOKAllasjelzesOffsetek[2] = 270; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[2][0] =1250;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[2][0] = 241;			/* Front end F */															/**/
...
/* 10. TMOK: 74-02 RTU: Front end D -> Pápakovácsi                	 -----------------------*/								/**/
TMOKAllasjelzesOffsetek[10] = 1250; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[10][0] =1250;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[10][0] = 126;			/* Pápakovácsi */															/**/
...
nReteszOffset[11] = 10;             /* DC parancs és reteszfunkció DP állapot offsete, ha tartozik hozzá DC parancs*/

/**********************************************************************************************************************/
/* 12. TMOK: 87-67 RTU: Front end D -> Bicske szeméttelep                	 -----------------------*/								/**/
TMOKAllasjelzesOffsetek[12] = 691; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[12][0] =691;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[12][0] = 233;			/* Bicske steméttelep */															/**/
...
nReteszOffset[12] = 11;             /* DC parancs és reteszfunkció DP állapot offsete, ha tartozik hozzá DC parancs*/

/**********************************************************************************************************************/
/* 13. TMOK: 15-52 RTU: Front end F -> Felpéc biogáz , H front end              	 -----------------------*/								/**/
TMOKAllasjelzesOffsetek[13] = 319; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/

TMOK_ID[13][0] =1254;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[13][0] = 241;			/* Front end F */															/**/
TMOK_ID[13][0] =1284;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[13][0] = 242;			/* Front end H */															/**/
...
/**********************************************************************************************************************/
/* 14. TMOK: 10-99 RTU: Front end F -> Ikrény biogáz               	 -----------------------*/								/**/
TMOKAllasjelzesOffsetek[14] = 273; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[14][0] =1255;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[14][0] = 241;			/* Front end F */															/**/
...
/**********************************************************************************************************************/
/* 15. TMOK: 13-07 RTU: Front end H -> Gyõr 0747/9 PV erõmû               	 -----------------------*/								/**/
TMOKAllasjelzesOffsetek[15] = 587; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[15][0] =1292;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[15][0] = 242;			/* Front end H */															/**/
...
/**********************************************************************************************************************/
/* 17. TMOK: 10-93 RTU: Front end H -> Gyõr 0452/6 Árki PV erõmû               	 -----------------------*/								/**/
TMOKAllasjelzesOffsetek[17] = 580; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[17][0] =1305;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[17][0] = 242;			/* Front end H */															/**/
...
nReteszOffset[17] = 16;             /* DC parancs és reteszfunkció DP állapot offsete, ha tartozik hozzá DC parancs*/
/**********************************************************************************************************************/
/* 18. TMOK: 15-54 RTU: Front end H -> Gyõr 0452/6 Árki PV erõmû               	 -----------------------*/								/**/
TMOKAllasjelzesOffsetek[18] = 320; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[18][0] =1307;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[18][0] = 242;			/* Front end H */															/**/
...
nReteszOffset[18] = 17;             /* DC parancs és reteszfunkció DP állapot offsete, ha tartozik hozzá DC parancs*/
/**********************************************************************************************************************/
/* 19. TMOK: 51-02 RTU: Front end H -> Szentkirályszabadja 067/6 PV erõmû               	 -----------------------*/								/**/
TMOKAllasjelzesOffsetek[19] = 93; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[19][0] =1308;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[19][0] = 242;			/* Front end H */															/**/
...
nReteszOffset[19] = 18;             /* DC parancs és reteszfunkció DP állapot offsete, ha tartozik hozzá DC parancs*/
/**********************************************************************************************************************/
